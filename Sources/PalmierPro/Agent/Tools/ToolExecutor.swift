...
/// Single source of truth for tool execution; the MCP server and the in-app
/// agent both dispatch here so the two surfaces never drift.
@MainActor
final class ToolExecutor {
    ...
    private func readMedia(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let mediaRef: String = try args.require("mediaRef")
        ...
        return ToolResult(
            content: [.image(base64: data.base64EncodedString(), mediaType: mime), .text(try jsonString(meta))],
            isError: false
        )
    }

    private func addTrack(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let typeStr: String = try args.require("type")
    ...
    private func removeTrack(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let trackId: String = try args.require("trackId")
    ...
    private func addClip(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let mediaRef: String = try args.require("mediaRef")
        let trackIndex: Int = try args.require("trackIndex")
        let startFrame: Int = try args.require("startFrame")
        let durationFrames: Int = try args.require("durationFrames")
    ...
    private func removeClip(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let clipId: String = try args.require("clipId")
    ...
    private func moveClip(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let clipId: String = try args.require("clipId")
        let toTrack: Int = try args.require("toTrack")
        let toFrame: Int = try args.require("toFrame")
        guard editor.findClip(id: clipId) != nil else { throw ToolError("Clip not found: \(clipId)") }
    ...
    private func splitClip(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        let clipId: String = try args.require("clipId")
        let atFrame: Int = try args.require("atFrame")
        guard let loc = editor.findClip(id: clipId) else { throw ToolError("Clip not found: \(clipId)") }
        let clip = editor.timeline.tracks[loc.trackIndex].clips[loc.clipIndex]
    ...
    private func generate(_ editor: EditorViewModel, _ args: [String: Any], type: ClipType) throws -> ToolResult {
        let prompt: String = try args.require("prompt")
        let gen = editor.generationService
        guard gen.hasApiKey else {
        ...
        if type == .video {
            ...
            if model.requiresSourceVideo {
                guard let sourceRef = args.string("sourceVideoMediaRef") else {
                    throw ToolError("Model '\(modelId)' requires 'sourceVideoMediaRef' pointing to a video asset.")
                }
                ...
                let placeholderId = gen.generate(
    ...
    private func listModels(_ args: [String: Any]) -> ToolResult {
        ...
        return (try? .ok(jsonString(out))) ?? .error("Failed to encode model list")
    }
    ...
    private func jsonString(_ obj: Any) throws -> String {
        guard let s = Self.jsonString(obj) else { throw ToolError("Failed to encode JSON") }
        return s
    }
...
extension Dictionary where Key == String, Value == Any {
    ...
    func require<T>(_ key: String) throws -> T {
        if T.self == String.self, let v = string(key) as? T { return v }
        if T.self == Int.self, let v = int(key) as? T { return v }
        if T.self == Double.self, let v = double(key) as? T { return v }
