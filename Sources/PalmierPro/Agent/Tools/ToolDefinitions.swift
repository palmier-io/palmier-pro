...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .trimClip,
            description: "Trims clip heads and tails as ONE exact, atomic, undoable edit — pass every trim in the trims array in a single call instead of one call per clip (e.g. retiming a whole caption track). Set ripple explicitly: ripple=true matches Shift-drag in the timeline, moving everything downstream (including sync-locked tracks) so clips stay butted; ripple=false matches a regular edge drag, where extension overwrites and shortens same-track neighbors while shortening leaves a gap. ripple=true accepts exactly one trim per call because it shifts downstream frames; ripple=false batches freely.\n\nEach trim names a clipId, an edge, and exactly one length mode:\n• deltaFrames — signed edge-relative length change. Positive extends at the named edge; negative shortens.\n• toFrame — exact project frame of the cut, resolved against the timeline as it is when the call starts. For a tail this is the new exclusive end. For a head it is the project frame whose content becomes first; regular trim also moves the head to that frame, while ripple trim keeps the clip's timeline start anchored.\n• extendToAdjacentClip=true — regular trim only. Extends through the empty gap to the nearest same-track clip on that edge and stops at its boundary without changing it. Reports a no-op if already touching; errors if no adjacent clip exists.\n\nMulticam: to move a camera cut or resize one angle segment, use ONE ripple=false trim — extending overwrites the adjacent angle while the group's continuous mic and program clock stay put. Do not shorten one angle and extend the other as two steps. Ripple trims are only accepted at a boundary the whole group shares.\n\nThe whole batch must land exactly or nothing changes: if bounded source media, a linked partner, retiming quantization, clip minimum length, or sync-locked room blocks any entry, the error names the entry and reports maxExtendFrames or the nearest achievable delta. A clip edge may appear once per call (a head and tail of the same clip may both appear; trims apply in array order). Linked audio/video partners trim together. Use ripple_delete_ranges for cuts from the middle of clips.",
            inputSchema: objectSchema(
                properties: [
                    "ripple": ["type": "boolean", "description": "Required. true shifts downstream clips (one trim per call); false overwrites on extension and leaves a gap on shortening (batchable)."],
                    "trims": [
                        "type": "array",
                        "description": "Edge trims to apply atomically in one undoable action, in order. Each entry: clipId, edge, and exactly one of deltaFrames, toFrame, or extendToAdjacentClip.",
                        "items": [
                            "type": "object",
                            "properties": [
                                "clipId": ["type": "string", "description": "Clip to trim. Linked partners trim with it."],
                                "edge": [
                                    "type": "string",
                                    "enum": ["head", "tail"],
                                    "description": "'head' is the in-point; 'tail' is the out-point.",
                                ],
                                "deltaFrames": ["type": "integer", "description": "Signed length change: positive extends, negative shortens."],
                                "toFrame": ["type": "integer", "description": "Exact project frame of the new cut, in pre-call timeline frames."],
                                "extendToAdjacentClip": ["type": "boolean", "description": "Set true to fill the empty gap to the nearest clip on this track without touching it. Regular trim only."],
                            ],
                            "required": ["clipId", "edge"],
                        ],
                    ],
                ],
                required: ["ripple", "trims"]
            )
        ),
        AgentTool(
            name: .setClipProperties,
            description: "Apply the same generic clip property values to one or more clips in a single undoable action. Pass any combination of durationFrames, trimStartFrame, trimEndFrame, speed, volumeDb, opacity, fades, edgeRounding, edgeSoftness, transform, or blendMode (video/image clips only). For text content, typography, captions, and text animation, use update_text.\n\nNOT for preview layout — split screen, picture-in-picture, grid, sidebar, and any multi-clip canvas arrangement belong to apply_layout, which sets transform and crop together. Do not use transform here (or set_keyframes position/scale/crop) to build those layouts.\n\nAll values apply to every clip in clipIds; for per-clip differences, make separate calls. trimStartFrame/trimEndFrame are offsets from the source media, not the timeline. speed 1.0 is normal, <1.0 slows (clip gets longer on the timeline), >1.0 speeds up. volumeDb is −60 through +15 dB; 0 dB keeps source level and −60 dB is mute. opacity is 0.0–1.0. fadeInFrames/fadeOutFrames are clip-relative lengths; 0 clears that fade, and their sum must fit within the resulting clip duration. Fades multiply existing opacity or volume keyframes instead of replacing them: visual/text clips fade opacity, while audio clips fade gain. Fades are per-clip and don't propagate to linked media — include both the visual clip id and its nested audio.id from get_timeline to fade picture and sound together. edgeRounding and edgeSoftness are 0.0–1.0, where 1 reaches half the shorter visible edge. transform is for rare single-clip tweaks only — 0–1 normalized canvas coords, partial merge; rotation is clockwise degrees; flipHorizontal/flipVertical mirror across the axis.\n\nFor moves and start-frame changes, use move_clips. For edge timing that matches a regular or ripple timeline trim, including overwrite, gap fill, multicam angles, and captions, use trim_clip. For animated values (keyframes), use set_keyframes — setting volumeDb, opacity, or transform.rotation here clears any existing keyframe track on that property.\n\nTiming changes (durationFrames, trimStartFrame, trimEndFrame, speed) on a linked clip carry over to its linked partner so audio/video stay in sync — same as the timeline UI. Per-clip fields (volumeDb, opacity, fades, edgeRounding, edgeSoftness, transform, blendMode) don't propagate. trim and speed are skipped for text partners.\n\nTiming fields (trims, durationFrames, speed) are refused on multicam clips because they would slip the clip out of sync. Use trim_clip to lengthen or shorten a multicam camera segment, change_cam for angles, or property fields for visual/audio changes.",
            inputSchema: objectSchema(
                properties: [
                    "clipIds": [
                        "type": "array",
