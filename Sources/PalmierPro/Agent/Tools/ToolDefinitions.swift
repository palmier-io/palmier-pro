...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .cropToSubject,
            description: "Crop a video or image clip to a visually described rectangular subject, such as \"the streamer's PIP\" or \"the receipt on the desk.\" This is a vision-guided preview/commit workflow; the tool does not guess coordinates by itself.\n\nFirst call with clipId and prompt only. It returns the raw source frame with a 10×10 A1–J10 grid. Identify the subject and submit approximate normalized bounds. Each preview with bounds returns three images in receipt imageRoles order: full-frame context with the proposal, a fresh 10×10 grid zoomed into that proposal, and a clean crop preview. Use grid.xEdges and grid.yEdges to convert the zoomed cells directly to tighter absolute source coordinates, then preview again as often as needed. Only the final call sets apply=true; that commits one undoable static crop and clears crop keyframes. Preview calls never mutate the timeline or create undo entries.\n\nBounds always remain absolute source coordinates from 0 to 1 with a top-left origin: left < right and top < bottom. Applying a crop makes it the clip's authored source framing; a later apply_layout preserves it and crops further only when its slot requires it. atFrame selects the representative timeline frame and must be inside the clip; omit it to use the midpoint. For moving subjects use set_keyframes after identifying bounds at multiple frames.",
            inputSchema: objectSchema(
                properties: [
        ...
        AgentTool(
            name: .applyLayout,
            description: "Arrange multiple clips into a common multi-video layout (split screen, picture-in-picture, grid) in one undoable action — the fast path for composing several videos in one frame. Use this instead of hand-setting transforms and screenshot-checking alignment with inspect_timeline.\n\nYou pick a named layout and assign a clip to each slot. Existing authored crops are treated as the clip's new source frame and are never replaced: fit='fill' (default) may crop that visible content further to cover the slot without stretching, while fit='fit' letterboxes all of the visible content. Reapplying a layout always recomputes from the authored crop, so layout crops do not accumulate. Clips with animated crop keyframes are refused because one static slot crop cannot preserve that animation exactly.\n\nThe layout crop is centered by default. When that chops off something important, bias which part survives: 'anchor' is a coarse shortcut, while anchorX/anchorY (0–1) give continuous control. To nudge framing after the fact, call apply_layout again with adjusted anchorX/anchorY.\n\nTwo modes (don't mix across slots):\n• Place new clips: give each slot a 'mediaRef' plus startFrame and endFrame. Creates one stacked video track per slot; PIP inset tracks are placed on top automatically.\n• Re-layout existing clips: give each slot 'clipIds'. Authored crop, timing, and tracks stay intact; current track order decides stacking.\n\nEvery slot of the chosen layout must be filled. Layouts and slots: full—main; side_by_side—left/right; top_bottom—top/bottom; pip_*—main/inset; grid_2x2, grid_3x3, grid_4x4—rNcN from top-left; main_sidebar—main/sidebar; three_up—left/center/right; three_stack—top/middle/bottom.",
            inputSchema: objectSchema(
                properties: [
                    "layout": [
                        "type": "string",
