...
enum ToolDefinitions {
    static let all: [AgentTool] = [
                ...
                properties: [
                    "entries": [
                        ...
                        "items": [
                            "type": "object",
                            "properties": [
                                "mediaRef": ["type": "string", "description": "ID of the media asset from get_media. Omit (or pass empty) when isAdjustment=true."],
                                ...
                                "endFrame": ["type": "integer", "description": "Optional. Occupy timeline frames [startFrame, endFrame) — a gap from get_timeline copies straight in. For stills, frame-exact fills, and adjustment layers. Mutually exclusive with source."],
                                ...
                                "isAdjustment": ["type": "boolean", "description": "Optional. When true, creates an adjustment layer — no media source, its effects apply to all clips below. Apply color/effect tools to this clip's ID."],
                            ],
                            "required": ["startFrame"],
                        ],
                    ],
        ...
        AgentTool(
            ...
            description: "Analyzes the rhythm and energy of an audio or video asset on-device and returns beat timestamps in both seconds and project frames. Use this before arranging video clips to music: the returned beatsInFrames array gives the exact frame values where cuts should land to stay on-beat. Returns: bpm (estimated tempo), beatIntervalFrames (frames per beat — use as durationFrames for one-beat clips), beats/beatsInFrames (every beat), downbeats/downbeatsInFrames (bar starts, every 4th beat — prefer these for major scene cuts), confidence (0–1, higher = steadier rhythm), beatCount, downbeatCount, climaxSeconds/climaxFrame (the song's energy peak — chorus/drop), and energy (smoothed 0–1 loudness curve, one value per stepSeconds, showing the song's build/quiet structure). To open a video with the music already at its climax, trim the music clip with trimStartFrame = climaxFrame (or a downbeat just before it). Runs entirely on-device; free and fast (typically under 2 seconds per song).",
            inputSchema: objectSchema(
                properties: [
