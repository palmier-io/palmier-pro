...
extension ToolExecutor {
    ...
    func duplicateClips(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        let linkedCount = moves.count - input.entries.count
        let linkedNote = linkedCount > 0 ? " (+\(linkedCount) linked)" : ""
        let summary = input.entries.map { "\($0.clipId) → \($0.toTrack.map { "track \($0)" } ?? "same track") @ \($0.toFrame)" }.joined(separator: "; ")
        return .ok("Duplicated \(input.entries.count) clip\(input.entries.count == 1 ? "" : "s")\(linkedNote): \(summary)")
    }
