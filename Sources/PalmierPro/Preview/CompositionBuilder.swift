...
enum CompositionBuilder {
    ...
    private final class BuildContext {
        ...
        init(
            ...
            self.showMaskOverlay = showMaskOverlay
    ...
    private static func insertVideoLane(
        ...
        for clip in clips {
            ...
            if await insertClip(clip, sourceAsset: source.asset, sourceTrack: source.track,
                ...
                previousEndFrame = clip.endFrame
                await insertMatteTracks(for: clip, ctx: ctx)
            }
        }
    ...
    /// Inserts one hidden matte track per enabled, tracked object mask.
    /// Matte frame `i` covers source frame `firstSourceFrame + i` — source
    /// position is timeline-fps frames, matte duration runs at the segment fps.
    private static func insertMatteTracks(for clip: Clip, ctx: BuildContext) async {
        guard let masks = clip.masks, !masks.isEmpty,
              clip.speed.isFinite, clip.speed > 0
        else { return }
        for mask in masks {
            guard mask.enabled,
                  mask.removesBackground || ctx.showMaskOverlay,
                  let track = mask.track,
                  track.mediaRef == clip.mediaRef,
                  track.fps > 0,
                  let matteURL = ctx.resolveMaskURL(track.id)
            else { continue }
            ...
            let visibleRange = CMTimeRange(
                start: visibleStart,
                duration: CMTimeMultiplyByFloat64(clipDuration, multiplier: clip.speed)
            )
            let matteStart = CMTime(
                value: CMTimeValue(track.firstSourceFrame),
                timescale: ctx.timescale
            )
            let matteDuration = CMTime(
                seconds: Double(track.frameCount) / track.fps,
                preferredTimescale: 600_000
            )
            let overlap = CMTimeRangeGetIntersection(
                visibleRange,
                otherRange: CMTimeRange(start: matteStart, duration: matteDuration)
            )
            guard overlap.isValid, !overlap.isEmpty, overlap.duration > .zero else { continue }

            let asset = ctx.makeAsset(matteURL)
            guard let sourceTrack = try? await ctx.loadTracks(asset, .video).first,
                  let compTrack = ctx.composition.addMutableTrack(
                      withMediaType: .video,
                      preferredTrackID: kCMPersistentTrackID_Invalid
                  )
            ...
            let artifactRange = CMTimeRange(start: overlap.start - matteStart, duration: overlap.duration)
            ...
            do {
                ...
                ctx.maskPlans[clip.id, default: []].append(
                    LayerPlan.MaskPlan(
                        maskId: mask.id,
                        trackID: compTrack.trackID,
                        showsOverlay: ctx.showMaskOverlay
                    )
                )
            } catch {
                ctx.composition.removeTrack(compTrack)
                Log.preview.warning("matte insert failed clipId=\(clip.id) maskId=\(mask.id): \(error.localizedDescription)")
            }
        }
    }
            ...
            for childClips in flat.videoTracks.reversed() {
                ...
                for clip in childClips where clip.durationFrames > 0 {
                    ...
                    if clip.mediaType == .text {
                        ...
                        children.append(LayerPlan(source: .track(slot.trackID), clip: clip, natSize: slot.natSize,
                                                  preferredTransform: slot.transform, masks: maskPlans[clip.id] ?? []))
                    }
                }
            }
        ...
        for track in timeline.tracks.reversed() where !track.hidden {
            ...
            for clip in track.clips.sorted(by: { $0.startFrame < $1.startFrame }) where clip.durationFrames > 0 {
                ...
                if clip.mediaType == .text {
                    ...
                    plan = LayerPlan(source: .track(slot.trackID), clip: clip, natSize: slot.natSize,
                                     preferredTransform: slot.transform, masks: maskPlans[clip.id] ?? [])
                    prevEndFrame = clip.endFrame
                }
