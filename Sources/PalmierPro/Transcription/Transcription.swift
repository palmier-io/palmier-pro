...
enum Transcription {
    ...
    static func transcribe(fileURL: URL, censorProfanity: Bool = false, preferredLocale: Locale? = nil, sourceRange: ClosedRange<Double>? = nil) async throws -> TranscriptionResult {
        ...
        Log.transcription.notice("transcribe locale=\(locale.identifier(.bcp47)) censor=\(censorProfanity)")
        ...
        Log.transcription.notice("ok textChars=\(decoded.text.count) words=\(decoded.words.count) segments=\(decoded.segments.count) lang=\(decoded.language ?? "?")")
        return decoded
    }
    ...
    static func extractAudioTrack(
        ...
        Log.transcription.notice("extract start video=\(videoURL.lastPathComponent) range=\(range != nil)")
