...
extension EditorViewModel {
    ...
    /// Deletes project-frame ranges from several tracks at once and closes the gaps in a single
    /// undoable swap. Unlike calling `rippleDeleteRangesOnTrack` per track, this never operates on
    /// stale frames: every track's ranges are resolved against the original layout, then all clears
    /// and sync-locked shifts are applied together. Refuses atomically (no partial edit) if a
    /// sync-locked follower can't absorb the shift, or if sync-locked tracks would be cut at
    /// different positions (which can't preserve their alignment).
    func rippleDeleteRangesOnTracks(_ rangesByTrack: [Int: [FrameRange]]) -> RippleRangesOutcome {
        var mergedByTrack: [Int: [FrameRange]] = [:]
        for (ti, ranges) in rangesByTrack {
            guard timeline.tracks.indices.contains(ti) else {
                return .refused("Track index out of range: \(ti)")
            }
            let merged = RippleEngine.mergeRanges(ranges.filter { $0.length > 0 })
            if !merged.isEmpty { mergedByTrack[ti] = merged }
        }
        guard !mergedByTrack.isEmpty else { return .refused("No non-empty ranges to delete") }

        // Regions to clear per track id: each track's own ranges plus the same spans on the tracks
        // holding linked A/V partners of any touched clip, so A/V stays in sync.
        var clearByTrackId: [String: [FrameRange]] = [:]
        for (ti, merged) in mergedByTrack {
            clearByTrackId[timeline.tracks[ti].id, default: []].append(contentsOf: merged)
            for clip in timeline.tracks[ti].clips
            where clip.linkGroupId != nil && merged.contains(where: { $0.start < clip.endFrame && $0.end > clip.startFrame }) {
                for pid in linkedPartnerIds(of: clip.id) {
                    if let l = findClip(id: pid) {
                        clearByTrackId[timeline.tracks[l.trackIndex].id, default: []].append(contentsOf: merged)
                    }
                }
            }
        }
        let clearTrackIds = Set(clearByTrackId.keys)

        // Sync-locked tracks must all ripple by the same ranges to stay aligned. Coincident cuts
        // (dual-mic / multicam of the same speech) and linked A/V partners share ranges and are
        // fine; genuinely divergent multi-track cuts can't preserve alignment without overlapping
        // or drifting, so refuse them rather than corrupt the layout. Non-sync-locked tracks opt
        // out of alignment and close their own gaps independently.
        let syncLockedClearedRanges: [[FrameRange]] = clearByTrackId.compactMap { tid, ranges in
            guard let ti = timeline.tracks.firstIndex(where: { $0.id == tid }),
                  timeline.tracks[ti].syncLocked else { return nil }
            return RippleEngine.mergeRanges(ranges)
        }
        if let first = syncLockedClearedRanges.first,
           syncLockedClearedRanges.dropFirst().contains(where: { $0 != first }) {
            return .refused("Can't cut words on multiple sync-locked tracks at different positions in one call without breaking their alignment. Remove words one track per call, re-reading get_transcript between calls.")
        }
        let alignmentRanges = syncLockedClearedRanges.first ?? []

        // Refuse up front if any sync-locked follower (a track with no clears of its own) can't
        // absorb the shift. Validating against the original layout is correct because the clears
        // below don't move these tracks' clips.
        for ti in timeline.tracks.indices {
            let track = timeline.tracks[ti]
            guard !clearTrackIds.contains(track.id), track.syncLocked, !alignmentRanges.isEmpty else { continue }
            let shifts = RippleEngine.computeRippleShiftsForRanges(clips: track.clips, removedRanges: alignmentRanges)
            if let reason = validateShifts(trackIndex: ti, shifts: shifts) { return .refused(reason) }
        }

        let totalRemoved = mergedByTrack.values.flatMap { $0 }.reduce(0) { $0 + $1.length }

        var shiftedClips = 0
        withTimelineSwap(actionName: "Ripple Delete") {
            for (tid, ranges) in clearByTrackId {
                guard let ti = timeline.tracks.firstIndex(where: { $0.id == tid }) else { continue }
                for r in RippleEngine.mergeRanges(ranges) {
                    clearRegion(trackIndex: ti, start: r.start, end: r.end, prune: false)
                }
            }
            for ti in timeline.tracks.indices {
                let track = timeline.tracks[ti]
                let removed: [FrameRange]
                if clearTrackIds.contains(track.id) {
                    // Sync-locked cleared tracks share alignmentRanges (== their own, by the check
                    // above); non-sync-locked cleared tracks close their own gaps.
                    removed = track.syncLocked ? alignmentRanges : RippleEngine.mergeRanges(clearByTrackId[track.id] ?? [])
                } else if track.syncLocked {
                    removed = alignmentRanges
                } else { continue }
                guard !removed.isEmpty else { continue }
                shiftedClips += applyShifts(RippleEngine.computeRippleShiftsForRanges(clips: track.clips, removedRanges: removed))
                sortClips(trackIndex: ti)
            }
        }

        let anchorTi = mergedByTrack.keys.min() ?? 0
        return .ok(RippleRangesReport(
            removedFrames: totalRemoved,
            clearedTracks: clearTrackIds.count,
            shiftedClips: shiftedClips,
